...
export class EskizService implements OnApplicationBootstrap {
  ...
  private async login() {
    try {
      ...
      if (!email || !password) {
        this.logger.warn(
          'Eskiz credentials not configured, skipping SMS service',
        );
        return;
      }
      ...
      let tokenData = await this.eskizRepository.findOne({
        where: { name: 'eskiz-token' },
      });
      if (tokenData) {
  ...
  async refreshToken() {
    try {
      const oldToken = await this.eskizRepository.findOne({
        where: { name: 'eskiz-token' },
      });
      if (!oldToken) {
        this.logger.warn('No Eskiz token found, SMS service disabled');
        return;
      }
      ...
      this.logger.warn('Eskiz token refresh failed, SMS service disabled');
    }
  }

  async sendSms(phone: string, message: string) {
    try {
      const tokenData = await this.eskizRepository.findOne({
        where: { name: 'eskiz-token' },
      });
      if (!tokenData) {
